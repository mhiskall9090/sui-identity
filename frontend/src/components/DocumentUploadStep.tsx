...
const DocumentUploadStep: React.FC<DocumentUploadStepProps> = ({ onNext, onBack, onFileUpload, documentType }) => {
  ...
  const handleDocumentUpload = async (file: File) => {
    ...
    try {
      ...
      const result = await handleApiCall('/api/identity/extract-document-data', formData);
      ...
      if (result.data) {
        ...
        setError(result.message || `Failed to process ${getDocumentTitle()} image.`);
      }
    } catch (err) {
      const errorMsg = err instanceof Error ? err.message : `An error occurred while processing the ${getDocumentTitle()} image`;
      setError(errorMsg);
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="space-y-6">
        ...
        {documentData && !error && (
          <div className="p-4 rounded-2xl" style={{ backgroundColor: `${colors.primary}10`, border: `1px solid ${colors.primary}30` }}>
            <div className="flex items-center gap-2 mb-3">
              ...
              <h4 className="font-semibold" style={{ color: colors.white }}>{getDocumentTitle()} Data Extracted</h4>
            </div>
            <div className="grid grid-cols-1 gap-2 text-sm">
              {documentData.document_number && (
                <div>
                  <span className="font-medium" style={{ color: colors.primary }}>{getDocumentTitle()} Number:</span>
                  <span className="ml-2" style={{ color: colors.lightBlue }}>{documentData.document_number}</span>
                </div>
              )}
              {documentData.phone_number && (
                <div>
                  <span className="font-medium" style={{ color: colors.primary }}>Phone Number:</span>
                  <span className="ml-2" style={{ color: colors.lightBlue }}>{documentData.phone_number}</span>
                </div>
              )}
              {documentData.dob && (
                <div>
                  <span className="font-medium" style={{ color: colors.primary }}>Date of Birth:</span>
                  <span className="ml-2" style={{ color: colors.lightBlue }}>{documentData.dob}</span>
                </div>
              )}
            </div>
          </div>
        ...
        {!previewUrl ? (
          <div className="border-2 border-dashed rounded-2xl p-8 text-center transition-colors"
            ...
            <h3 className="text-lg font-semibold mb-2" style={{ color: colors.white }}>Upload {getDocumentTitle()}</h3>
            <p className="mb-6" style={{ color: colors.lightBlue }}>Choose a clear image of your {getDocumentTitle()} (JPG, PNG)</p>
          ...
          <div className="text-center">
            ...
            <button
              type="button"
              onClick={() => {setPreviewUrl(null); setDocumentData(null); setError(null);}}
